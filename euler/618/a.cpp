...
ll S(int k, int m) {
    ...
    FOR(i, m, sz(primes)) {
        ...
        //ret += (S(k - primes[i], i) * primes[i]) % MOD;
        //ret %= MOD;
        ret += S(k - primes[i], i) * primes[i];
    }
    mem[k][m] = ret;
    return ret;
}
...
void Solve()
{
    ...
    fib[0] = 1;
    fib[1] = 1;
    FOR(i, 2, 24) fib[i] = fib[i-1] + fib[i-2];

    //FOR(i, 0, 100000) if (primes[i] > 46368) {
    //    cout << i << endl;
    //    break;
    //}
    //exit(0);

    cout << "largest:" << fib[23] << endl;
    FOR(i, 0, fib[23]) FOR(j, 0, 5000) mem[i][j] = -1;
